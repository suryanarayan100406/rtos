...
- **"Metric without GCPs" is honest, not magic.** Scale comes from the Inertial Measurement Unit (IMU)
  plus GNSS baselines; the datum comes from GNSS(+RTK/PPK). We report **two regimes**: **(A) RTK/PPK ≈
  centimetre-level** and **(B) GPS-only ≈ sub-metre** — never GCP-grade survey accuracy for free from
  plain GPS (see the honesty guardrails in [`PROBLEM_STATEMENT`](../_internal/PROBLEM_STATEMENT.md) §4).
...
imagery. It is *injected* from sensors: on-manifold IMU pre-integration (Forster et al.) observes
gravity direction and metric scale between keyframes, and GNSS baselines give an independent metric
distance. This is exactly what removes the monocular scale ambiguity that photogrammetric
self-calibration cannot resolve on one strip.
...
flowchart LR
  subgraph SPINE["Metric spine (S2/S6) — factors co-owned with Systems"]
    IMU["IMU pre-integration: scale + gravity"] --> FG(("Factor graph GTSAM / iSAM2"))
    VIS["Visual keyframe factors"] --> FG
    GNSS["GNSS factors (robust Huber/DCS)"] --> FG
...
- **GPS-only is regime B, not regime A.** With plain single-frequency GPS the alignment degrades to a
  Sim3 fit against metre-level positions: **sub-metre absolute** (design target H 0.5–2 m, V 0.8–2.5 m
  per spec §12), with decimetre-level *relative/local* accuracy preserved by the IMU and visual
  geometry. This is useful and honest — it is not centimetre survey accuracy, and we never label it so.
...
a recognised standard rather than assert it. Everything here maps to the Evaluation Criteria table in
[`PROBLEM_STATEMENT`](../_internal/PROBLEM_STATEMENT.md) §3 and the accuracy budget in
[`CANONICAL-ARCHITECTURE-SPEC`](../_internal/CANONICAL-ARCHITECTURE-SPEC.md) §12.
...
| # (PROBLEM_STATEMENT) | Criterion | How measured | Target — A: RTK/PPK · B: GPS-only |
|---|-----------|--------------|-----------------------------------|
| 1 | Absolute geometric accuracy | H & V RMSE vs independent checkpoints; ASPRS Ed.2 95% (H×1.7308, V×1.96) | A: H 3–8 cm, V 5–12 cm (+1×GSD) · B: H 0.5–2 m, V 0.8–2.5 m, with CI |
| 2 | Relative accuracy | Known-distance / scale-bar error; local RMSE | A: <1% of distance · B: report measured dm-level value |
| 3 | GSD | cm/px from AGL & sensor geometry | ~1.5–3 cm/px (~2 cm/px @ 80 m AGL), both regimes |
| 4 | Completeness / coverage | % surface above confidence threshold; occlusion-flagged area | Maximise; **explicitly report** unseen / low-confidence regions |
| 5 | Reconstruction fidelity | Point density (pts/m²), hole ratio, texture sharpness | Quantitative + qualitative panel (feeds report; geometry owned by Recon Lead) |
| 6 | Quality vs reference | C2C, C2M, Chamfer / M3C2 vs COLMAP / Metashape / LiDAR | Minimise distance; report percentiles |
| 10 | Georeferencing correctness | Position error of known features; EPSG + datum + geoid check | Within budget; correct EPSG + geoid, both regimes |

All figures in the A/B columns are **design targets to be measured against checkpoints**, consistent
with spec §12; external comparisons are quoted **as reported**.
...
| Textured mesh (multi-LOD) | **glTF/GLB, OBJ+MTL, OGC 3D Tiles 1.1** | 3D Tiles boundingVolume + metadata carry CRS |
...
tiling; py3dtiles + CesiumJS and Potree for 3D Tiles and point-cloud web serving. These match the
Geospatial I/O and viewer choices in [`CANONICAL-ARCHITECTURE-SPEC`](../_internal/CANONICAL-ARCHITECTURE-SPEC.md)
§7 — no alternative names are introduced.
...
| Geospatial factors in fusion | GTSAM 4.2 (RTK/GNSS + IMU + baro factors, iSAM2) — **co-owned with Systems** | Ceres / g2o; robust Huber / DCS |
...
(2) *Report two regimes (A/B) and a per-region confidence, never one global number.* (3) *Always convert
to orthometric height and always embed CRS + geoid* — datum safety is validated at startup and the
pipeline refuses to silently mix datums. (4) *Favour permissive licensing* (GTSAM is BSD; RTKLIB, PROJ,
GDAL, PDAL are permissive) for defense deployability — the licensing caveat is tracked with the
[AI / Deep Learning](3-ai-deep-learning-research.md) role.
...
flowchart TD
  ...
  Q -->|"single / float GNSS"| B["Regime B · Sim3 to GPS + GNSS-prior BA<br/>sub-metre absolute · strong relative (L1)"]
...
- **Georeference-on-reacquire (L2).** On GNSS dropout (urban canyon, jamming, denied environment) the
  factor graph bridges the gap with IMU + VIO dead-reckoning and holds a **local metric ENU** map. When
  GNSS returns, the `map`→`earth` transform re-anchors via new global factors and the trajectory is
  re-georeferenced — no data is discarded, the gap is simply flagged as higher-uncertainty.
...
4. **Demo both accuracy regimes** by toggling RTK versus GPS-only (inject GPS noise), showing regime A
   ...
   graceful-degradation and honesty story made visible to judges.
...
- **Vertical accuracy without a checkpoint.** A systematic ~10–30 cm vertical bias
  ...
  precise local geoid + calibrated boresight is effectively required for sub-decimetre vertical. Honest
  GPS-only budget stays sub-metre.
